...
// VARIABLES GLOBALES
    ...
    $.fn.obtenerStockUniPorCodpro = function (codpro) {
        ...
        $('#table-central').DataTable().rows().every(function () {
    ...
    $.fn.listarTablaDistribucion = function (codpro, despro) {
        ...
        if (codpro !== null) {
            ...
            if (despro !== undefined) {
                ...
                $('#datosproducto').html(cadena + despro + "</b><col>, Codigo: <b>" + codpro + "</b></col></row>");
            }
            ...
            tabla = $('#table-establecimientos').DataTable({
                ...
                language: {
                    ...
                    data: function (d) {
                        d.tipoStkMin = dias,
                                d.tipo_distrib = tipoDistrib, d.mostrar0estab = mostrar0estab,
                                d.codpro = codpro, d.mostrarrojos = mostrarrojos, d.soloalm = soloalm,
                                d.codtip = codtip, d.codalm = codalm, d.distpor = distpor, d.multiplicador = 1;
                    },
                    beforeSend: function () {
                    ...
                    dataSrc: function (json) {
                        ...
                        dataCopy.forEach(item => {
                            if (item.restringido === 'N' && item.inmov === 'N') {
                                ...
                                let formula = 0;
                                formula = ((item.stkalm + item.transE) * stockFra + (item.stkalm_m + item.transF)) / blister;

                                let promventa = parseFloat(item.stkmin2);
                                //let compr = formula / tableta;
                                valor = Math.ceil((promventa * tableta) - formula);
                                /*if (promventa > 0) {
                                 if (compr / promventa < multiplicador) {
                                 valor = Math.ceil((promventa * tableta) - formula);
                                 }
                                 }*/
                                if (valor > 0) {
                                    if (stockFra === 1 || item.aplicfrac === undefined || item.aplicfrac === 'N') {
                                        valor = Math.ceil(valor / stockFra) * stockFra;
                                    }
                                    if (item.masterpack !== undefined) {
                                        valor = Math.ceil(valor / (master * stockFra)) * master * stockFra;
                                    }
                                ...
                                if (a) {
                                    let cantidadlote = (Math.floor(item.qtymov * stockFra + item.qtymov_m) / blister);
                                    if (stockFra === 1 || item.aplicfrac === undefined || item.aplicfrac === 'N') {
                                        cantidadlote = Math.floor(cantidadlote / stockFra) * stockFra;
                                    }
                                    if (item.masterpack !== undefined) {
                                        cantidadlote = Math.floor(cantidadlote / (master * stockFra)) * master * stockFra;
                                    }
                                    totalesPorLote.push({lote: lote, cantidad: cantidadlote * blister});
                        ...
                        console.log(maximadistribucion);
                        console.log(maximorequerido);
                ...
                columns: [
                    ...
                    {data: 'stkalm',
                        "render": function (data, type, row) {
                            ...
                            if (parseFloat(row.stkmin2) > 0) {
                                let stock = row.stkalm + row.transE + (parseFloat(row.stkalm_m + row.transF) / stockFra);
                    ...
                    {data: null,
                        render: function (data, type, row) {
                            ...
                            let noaplicfrac = (row.aplicfrac === undefined || row.aplicfrac === 'N');
                            formula = ((row.stkalm + row.transE) * stockFra + (row.stkalm_m + row.transF)) / blister;
                            //let compr = formula / tableta;
                            let valor = 0;
                            ...
                            if (stockFra > 1 && !noaplicfrac) {
                                let formula = 0;
                                formula = (parseFloat(row.stkalm + row.transE) * stockFra + (parseFloat(row.stkalm_m + row.transF))) / blister;
                                let valor = 0;
                                valor = parseFloat(row.stkmin2) * tableta - formula;
                                console.log(row.sisent + ": " + valor);
                            ...
                            if (valore > 0) {
                                cadena += '<input type="text" size="5" value="' + valore + '" disabled>';
                            } else {
                    ...
                    {data: null,
                        render: function (data, type, row) {
                                ...
                                if (dataLista[row.codpro] && dataLista[row.codpro][row.siscod] && dataLista[row.codpro][row.siscod][row.lote] && dataLista[row.codpro][row.siscod][row.lote]["cantE"] !== undefined) {
                                    ...
                                    if (sugeridos[row.siscod] && sugeridos[row.siscod][row.lote] !== undefined && !dataLista[row.codpro] && Math.ceil(sugeridos[row.siscod][row.lote] / stockFra) > 0) {
                                        if (llenarautomaticamente === 'S') {
                                            actualizarbtn = true;
                                            if (stockFra > 1 && !noaplicfrac && Math.floor(sugeridos[row.siscod][row.lote] / stockFra) > 0) {
                                                cadena += '<input type="text" class="cantE" id="inputcantE"  value="' +
                                                        Math.floor(sugeridos[row.siscod][row.lote] / stockFra) + '" data-codpro="' +
                                                        row.codpro + '" data-codalm="' + row.siscod + '" data-lote="' + row.lote +
                                                        '" size="5" data-coscom="' + row.coscom + '" data-igvpro="' + row.igvpro + '" autocomplete="off">';
                                            } else if ((stockFra === 1 || noaplicfrac) && Math.ceil(sugeridos[row.siscod][row.lote] / stockFra) > 0) {
                                                cadena += '<input type="text" class="cantE" id="inputcantE"  value="' +
                            ...
                            if (data.restringido === 'N' && stockFra !== 1 && data.inmov === 'N') {
                                if (dataLista[row.codpro] && dataLista[row.codpro][row.siscod] && dataLista[row.codpro][row.siscod][row.lote] && dataLista[row.codpro][row.siscod][row.lote]["cantF"] !== undefined) {
                                    ...
                                    if (sugeridos[row.siscod] && sugeridos[row.siscod][row.lote] !== undefined && !dataLista[row.codpro] && sugeridos[row.siscod][row.lote] % stockFra > 0) {
                                        if (llenarautomaticamente === 'S' && stockFra > 1 && !noaplicfrac) {
                                            actualizarbtn = true;
    ...
    $.fn.calcularStock = function (codpro) {
        ...
        let valorMaximo = $.fn.obtenerMaximoPorCodpro(codpro) * valorStock + $.fn.obtenerStockUniPorCodpro(codpro);
        let sumas = 0;
        let sumasPorLote = {};
        let sumascantE = 0;
        let sumascantF = 0;
    ...
    $.fn.formatofecha = function (fechaISO) {
        ...
        let fechaFormateada = `${padLeft(fecha.getDate())}/${padLeft(fecha.getMonth() + 1)}/${fecha.getFullYear()} ${padLeft(fecha.getHours())}:${padLeft(fecha.getMinutes())}:${padLeft(fecha.getSeconds())}.${padRight(fecha.getMilliseconds(), 3)}`;
    ...
    $.fn.ListarTablaCentral = function () {
        ...
        tabla = $("#table-central").DataTable({
            ...
            language: {
                ...
                url: 'distribucion?opcion=6',
                type: 'POST',
                data: function (d) {
                    d.tipoStkMin = dias, d.tipoDistrib = tipoDistrib, d.codtip = codtip, d.codalm = codalm, d.soloalm = soloalm, d.multiplicador = multiplicador,
                            d.secuencia = secuencia, d.indicaFecha = indicaFecha, d.fecha1 = fecha1, d.solorojos = mostrarrojoscentral, d.distpor = distpor, d.mostrar0central = mostrar0central, d.generico = generico, d.objgenerico = objgenerico;
                },
                beforeSend: function () {
...
//                },
                ...
                {data: null,
                    render: function (data, type, row) {
                        if (dataLista[row.codpro]) {
                            return '<button class="btn btn-warning btn-sm" data-tienerojos="' + row.tienerojos + '"  data-codpro="' + row.codpro + '" data-despro="' + $('<div>').text(row.despro).html() + '"><i class="fas fa-edit"></i></button>';
                        } else {
                            if (row.tienerojos === "S") {
                                return '<button class="btn btn-danger btn-sm" data-tienerojos="' + row.tienerojos + '"  data-codpro="' + row.codpro + '" data-despro="' + $('<div>').text(row.despro).html() + '"><i class="fas fa-edit"></i></button>';
                            } else {
                                return '<button class="btn btn-primary btn-sm" data-tienerojos="' + row.tienerojos + '"  data-codpro="' + row.codpro + '" data-despro="' + $('<div>').text(row.despro).html() + '"><i class="fas fa-edit"></i></button>';
                            }
                        }
                    }
                }
    ...
    $.fn.ListarTablaGenericos = function () {
        $("#contenedor-producto").hide();
        $("#contenedor-generico").show();
        $("#atras").hide();
        $.fn.validarSession();
        ...
        tabla = $("#table-generico").DataTable({
            ...
            language: {
                ...
                //url: 'distribucion?opcion=2&tipoStkMin='+dias+'&tipo_distrib='+codpro+'&mostrarrojos='+mostrarrojos,
                url: 'distribucion?opcion=5',
                type: 'POST',
                data: function (d) {
                    d.tipoStkMin = dias, d.tipoDistrib = tipoDistrib, d.codtip = codtip, d.codalm = codalm, d.soloalm = soloalm, d.multiplicador = multiplicador, d.generico = generico,
                            d.secuencia = secuencia, d.indicaFecha = indicaFecha, d.fecha1 = fecha1, d.solorojos = mostrarrojoscentral, d.distpor = distpor, d.mostrar0central = mostrar0central;
                },
                beforeSend: function () {
                    $("#loadingcentral").css("display", "block");
                }, complete: function () {
            ...
            columns: [
                ...
                {data: null,
                    render: function (data, type, row) {
                        if (dataLista[row.codpro]) {
                            return '<button class="btn btn-warning btn-sm" data-tienerojos="' + row.rojos + '"  data-codgen="' + row.codgen + '" data-desgen="' + $('<div>').text(row.desgen).html() + '"><i class="fas fa-edit"></i></button>';
                        } else {
                            if (row.tienerojos === "S") {
                                return '<button class="btn btn-danger btn-sm" data-tienerojos="' + row.rojos + '"  data-codgen="' + row.codgen + '" data-desgen="' + $('<div>').text(row.desgen).html() + '"><i class="fas fa-edit"></i></button>';
                            } else {
                                return '<button class="btn btn-primary btn-sm" data-tienerojos="' + row.rojos + '"  data-codgen="' + row.codgen + '" data-desgen="' + $('<div>').text(row.desgen).html() + '"><i class="fas fa-edit"></i></button>';
                            }
                        }
                    }
                }
    ...
    $("#table-generico").on("click", ".btn", function () {
        objgenerico = String($(this).data("codgen"));
        objname = String($(this).data("desgen"));
        $.fn.ListarTablaCentral();
    });
    $("#atras").click(function () {
        $.fn.ListarTablaGenericos();
    });
    $.fn.descargar = function (cabecera2) {
        ...
        if (Object.keys(dataLista).length > 0) {
            ...
            for (let siscod in objetoInvertido) {
                ...
                for (let codpro in codpros) {
                    ...
                    for (let lote in lotes) {
                        ...
                        if ((cantE !== undefined && cantE > 0) || (cantF !== undefined && cantF > 0)) {
            ...
            if (primerosIndices.length === 0) {
                ...
                console.log(primerosIndicesJSON);
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            for (let clavePrincipal in objetoInvertido) {
                                cadena = "";
                                let cabeceraesp = cabecera.filter(elemento => elemento.siscod_d === parseInt(clavePrincipal));
                                ...
                                for (let clavesecundaria in productos) {
                                    ...
                                    for (let lote in lotes) {
                                        ...
                                        if (datosExtraproductos[clavesecundaria]["coscom"] !== undefined) {
                                            if (dataLista[clavesecundaria][clavePrincipal][lote]["cantE"] !== undefined) {
                                                valpar = dataLista[clavesecundaria][clavePrincipal][lote]["cantE"] * datosExtraproductos[clavesecundaria]["coscom"];
                                            }
                                            sumvalpar += valpar;
                                            if (datosExtraproductos[clavesecundaria]["igvpro"]) {
                                                igvpar = valpar * datosExtraproductos[clavesecundaria]["igvpro"];
                                                sumigvpar += igvpar;
                                            }
                                ...
                                for (let clavesecundaria in productos) {
                                    let lotes = productos[clavesecundaria];
                                    for (let lote in lotes) {
                                        console.log("primeros indices: " + parseInt(clavePrincipal) + " productos: " + parseInt(clavesecundaria) + " lote: " + lote);
                                        let detalleesp = detalle.filter(elemento => elemento.siscod === parseInt(clavePrincipal) && elemento.codpro === clavesecundaria && elemento.lote === lote);
                                        let cantE = lotes[lote].cantE;
                                        let cantF = lotes[lote].cantF;
                                        if ((cantE !== undefined && cantE > 0) || (cantF !== undefined && cantF > 0)) {
                                            ...
                                            cadena = cadena + (parseInt(cabeceraesp[0].invnum) + numero) + "\t"; //agregamos invnum
                                            ...
                                            cadena = cadena + $.fn.formatonumero(detalleesp[0].vvfsal) + "\t"; //agregamos vvfsal
                                            cadena = cadena + $.fn.formatonumero(detalleesp[0].pvfsal) + "\t"; //agregamos pvfsal
                                            cadena = cadena + $.fn.formatonumero(detalleesp[0].pvpsal) + "\t"; //agregamos pvpsal
                                            ...
                                            //cadena = cadena + $.fn.formatofecha(detalleesp[0].fecven) + "\t";//agregamos fecven
                                            cadena = cadena + datosExtralotes[clavesecundaria][lote]["fecven"] + "\t"; //agregamos fecha vencimiento
                                            cadena = cadena + $.fn.formatonumero(detalleesp[0].util_vta) + "\t"; //agregamos util_vta
                                            cadena = cadena + $.fn.formatonumero(detalleesp[0].dscto_vta) + "\t"; //agregamos dscto_vta
                                            cadena = cadena + (parseInt(cabeceraesp[0].invnum) + numero) + "\t"; //agregamos invnum_c
                                            ...
                                            cadena = cadena + (parseInt(cabeceraesp[0].numdoc) + numero) + "\t"; //agregamos numdoc
                                            cadena = cadena + cabeceraesp[0].purnum + "\t"; //agregamos purnum
                                            cadena = cadena + $.fn.formatofecha(cabeceraesp[0].fecdoc) + "\t"; //agregamos fecdoc
                                            cadena = cadena + $.fn.formatofecha(cabeceraesp[0].fecpag) + "\t"; //agregamos fecpag
                                            ...
                                            cadena = cadena + (parseInt(cabeceraesp[0].invkar) + numero) + "\t"; //agregamos invkar
                                            cadena = cadena + $.fn.formatofecha(cabeceraesp[0].fecmov) + "\t"; //agregamos fecmov
                                ...
                                if (!nuevoarc) {
                                    ...
                                    let nombreArchivo = "DESCARGOENVIO_" + clavePrincipal + "_" + fechaHoraActual + "_" + (parseInt(cabeceraesp[0].invnum) + 1) + ".txt";
                        ...
                        error: function (jqXHR, textStatus, errorThrown) {
                            if (jqXHR.status === 400) {
                                alert('Error: Demasiados archivos por generar, Debe hacer una limpieza para continuar');
                            } else {
    ...
    $.fn.leerCantE = function () {
        $('#table-establecimientos').DataTable().rows().every(function () {
            ...
            if (!isNaN(data)) {
                ...
                if (dataLista[valcodpro] && dataLista[valcodpro][valcodalm] && dataLista[valcodpro][valcodalm][vallote] && dataLista[valcodpro][valcodalm][vallote]["cantE"] !== undefined) {
                    ...
                    if (!dataLista[valcodpro][valcodalm][vallote]) {
                        ...
                        datosExtralotes[valcodpro][vallote]["fecven"] = $.fn.obtenerFecVenPorLote(vallote);
                    }
                    dataLista[valcodpro][valcodalm][vallote]["cantE"] = data;
                }
            } else {
    ...
    $.fn.leerCantF = function () {
        $('#table-establecimientos').DataTable().rows().every(function () {
            ...
            if (!isNaN(data)) {
                // Comprobar si ya existe un registro con valcodpro y valcodalm iguales
                if (dataLista[valcodpro] && dataLista[valcodpro][valcodalm] && dataLista[valcodpro][valcodalm][vallote] && dataLista[valcodpro][valcodalm][vallote]["cantF"] !== undefined) {
                    // Si existe, reemplazar el valorInput
...
// Si no está marcado, deshabilitar el campo de fecha
    ...
    $("#btnAceptar").click(function () {
        if (idsSeleccionados.length !== 0 || $("#selectFiltro").val() === "SECUENCIA" || $("#selectFiltro").val() === "TODOS") {
            let comp = $("#selectStockMinimo").val() !== "" && $("#selectStockMinimoproyeccion").val() !== "";
            if (comp || (tempbtn === "REPOS")) {
                let comp = $("#selectStockMinimo2").val() !== "" && $("#selectStockMinimo2proyeccion").val() !== "";
                    ...
                    if ($("#inputSecuencia").val() !== "" || $("#selectFiltro").val() !== "SECUENCIA") {
                        if (estabSeleccionados.length !== 0 || $("#selectalm").val() === "TODOS") {
                            ...
                            if (distpor === "STKMIN") {
                                ...
                                $("#texto0estab").text("Mostrar establecimientos con stock minimo cero");
                                //multiplicador = 1;
                                ...
                                $("#texto0estab").text("Mostrar establecimientos con ventas cero");
...
// Remover el ID de la matriz JSON
    ...
    $('#table-establecimientos').on('change', '.cantE', function () {
        ...
        var codpro = String($(this).data('codpro'));
        var codalm = $(this).data('codalm');
        var lote = String($(this).data('lote'));
...
// Iterar sobre todas las filas en la tabla
        $.fn.leerCantE();
        if ($.fn.calcularStock(codpro)) {
            alert('El valor ingresado excede el Stock maximo permitido para este lote: ' + lote + '.');
            $(this).val("");
            delete dataLista[codpro][codalm][lote]["cantE"];
        }
        if (dataLista[codpro][codalm][lote]["cantE"] === undefined && dataLista[codpro][codalm][lote]["cantF"] === undefined) {
    ...
    $('#table-establecimientos').on('change', '.cantF', function () {
        ...
        // Obtener el valor de codpro del input actual
        var codpro = String($(this).data('codpro'));
        var codalm = String($(this).data('codalm'));
        var lote = String($(this).data('lote'));
        // Obtener el valor actual del input
        var valorInput = $(this).val();
        // Verificar si el valor ingresado es un número
...
// Iterar sobre todas las filas en la tabla
        $.fn.leerCantF();
        /*
         if (dataListaF.hasOwnProperty(codpro)) {
         var valcodproData = dataListaF[codpro]; // Obtener los datos asociados a valcodpro especificado
         
         // Iterar sobre las claves (valcodalm) del valcodpro especificado
         for (var valcodalm in valcodproData) {
         if (valcodproData.hasOwnProperty(valcodalm)) {
         sumas = sumas + valcodproData[valcodalm];
         }
         }
         }*/
        if ($.fn.calcularStock(codpro)) {
            alert('El valor ingresado excede el Stock maximo permitido para este producto: ' + lote + '.');
            $(this).val("");
...
// Verificar si el checkbox está marcado
    ...
    /*
     $('#solorojoscentral').change(function () {
     // Verificar si el checkbox está marcado
     if ($(this).is(':checked')) {
     $("#divsolo0central").hide();
     } else {
    ...
    $('#selectalm').on('change', function () {
        ...
        if (selectedOption === 'ESTAB') {
            ...
            $.getJSON("CRUDFaAlmacenes", parametro, function (resp) {
                ...
                let data = resp.data.filter(producto => !idsAEliminar.includes(producto.codalm));
                tabla = $("#tableestablecimientofiltro").DataTable({
                    ...
                    columns: [
                        ...
                        {data: null,
                            render: function (data, type, row) {
                                return '<input type="checkbox" class="checkbox" value="' + row.codalm + '">';
                            }}
                    ]
                });
            }).fail(function (jqXHR, textStatus, errorThrown) {
